...
function formatTime(dt, format) {
  ...
  const diff2 = now2.toMillis() - dt.toMillis();
  if (diff2 >= 0 && diff2 < 3e4) return "just now";
  if (diff2 >= 0 && diff2 < SEVEN_DAYS_MS) return dt.toRelative() ?? "";
...
function hydrateTimeElements() {
  ...
  for (const el of els) {
    ...
    if (!(dt == null ? void 0 : dt.isValid)) continue;
