const images = {
  // ── Page-level ──────────────────────────────────────────
  title: "沙盒",
  description: "moltis 缓存的用于沙盒执行的容器镜像。你可以删除单个镜像或清理全部。使用基础镜像和 apt 包构建自定义镜像。",
  appleContainerNote: "Apple Container 提供 VM 隔离执行，但不支持构建镜像。需要在 Apple Container 旁安装 Docker（或 OrbStack）来构建和缓存自定义镜像。沙盒命令通过 Apple Container 运行；镜像构建使用 Docker。",
  ...
  recommendation: {
    ...
    macosDockerTip: "Apple Container 在 macOS 26+ 上提供更强的 VM 级隔离。安装后会自动使用（moltis 优先选择它而非 Docker）。运行：brew install container",
