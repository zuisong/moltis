const mcp = {
  ...
  introDetail: "每个工具在你的机器上作为 <strong>本地进程</strong> 运行（通过 npm/uvx 启动）。Moltis 通过 stdio 连接到它，进程使用你的令牌代你发起外部 API 调用。数据不会发送到第三方 MCP 主机。",
  // ── Security warning ────────────────────────────────────
  securityTitle: "⚠️ MCP 服务器作为本地进程运行 — 启用前请仔细审查",
  securityPrivileges: "每个 MCP 服务器以 <strong>你的完整系统权限</strong> 运行。恶意或被入侵的服务器可以读取你的文件、窃取凭据或执行任意命令 — 就像任何本地进程一样。",
  securityReview: "<strong>在启用任何 MCP 服务器之前，请仔细检查其源代码</strong>。仅安装来自你信任的作者的服务器，并保持更新。",
